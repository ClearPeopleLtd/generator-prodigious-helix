/// <binding AfterBuild='Publish-Assemblies' ProjectOpened='sync' />
...
require("./buildtasks/Setup-Solr-Config.js");
...
gulp.task("Publish-Assemblies", function () {
    var root = "./src";
    var binFiles = root + "/**/code/**/bin/{Sitecore,CP}.{Feature,Foundation,Habitat," + config.mainProjectName + "}.*.{dll,pdb}";
...
gulp.task("Publish-All-Views", function () {
    var root = "./src";
    var roots = [root + "/**/Views", "!" + root + "/**/obj/**/Views", "!" + root + "/**/bin/**/Views"];
    var files = "/**/*.cshtml";
...
gulp.task("Publish-All-Configs", function () {
    var root = "./src";
    var roots = [root + "/**/App_Config", "!" + root + "/**/obj/**/App_Config", "!" + root + "/**/bin/**/App_Config"];
    var files = "/**/*.config";
...
gulp.task("Auto-Publish-Css", function () {
    var root = "./src";
    var roots = [root + "/**/styles", "!" + root + "/**/assets/style", "!" + root + "/**/obj/**/styles", "!" + root + "/**/bin/**/styles"];
    var files = "/**/*.css";
...
gulp.task("Auto-Publish-Scripts", function () {
    var root = "./src";
    var roots = [root + "/**/scripts", "!" + root + "/**/assets/scripts", "!" + root + "/**/obj/**/scripts", "!" + root + "/**/bin/**/scripts", "!" + root + "/**/bower_components/**"];
    var files = "/**/*.*";
    var destination = config.websiteRoot + "\\scripts";
    gulp.src(roots, { base: root }).pipe(
    foreach(function (stream, rootFolder) {
        gulp.watch(rootFolder.path + files, function (event) {
            console.log("Auto-Publish-Scripts");
            if (event.type === "changed") {
                gulp.src(event.path, { base: rootFolder.path }).pipe(gulp.dest(destination));
                console.log("published " + event.path);
            }
...
gulp.task("Auto-Publish-Images", function () {    
    var root = "./src";
    var roots = [root + "/**/assets/img"];
    var files = "/**/*.*";
    var destination = config.websiteRoot + "\\assets\\img";
    gulp.src(roots, { base: root }).pipe(
      foreach(function (stream, rootFolder) {
        gulp.watch(rootFolder.path + files, function (event) {
              if (event.type === "changed") {
...
gulp.task("Auto-Publish-Services", function () {    
    var root = "./src";
    var roots = [root + "/**/assets/services"];
    var files = "/**/*.*";
    var destination = config.websiteRoot + "\\assets\\services";
    gulp.src(roots, { base: root }).pipe(
      foreach(function (stream, rootFolder) {
        gulp.watch(rootFolder.path + files, function (event) {
              if (event.type === "changed") {
...
gulp.task("Auto-Publish-Fonts", function () {
    var root = "./src";
    var roots = [root + "/**/assets/fonts"];
    var files = "/**/*.*";
    var destination = config.websiteRoot + "\\assets\\fonts";
    gulp.src(roots, { base: root }).pipe(
      foreach(function (stream, rootFolder) {
        gulp.watch(rootFolder.path + files, function (event) {
              if (event.type === "changed") {
...
gulp.task("Auto-Publish-Assemblies", function () {
    var root = config.Paths.RootFolder;
    var roots = [root + "/**/code/bin"];
    //var files = "/{Sitecore,CP}.{Feature,Foundation,Habitat," + config.mainProjectName + "}.*.{dll,pdb}";
    var files = "\\*.dll";
    //var files = "\\*.{dll,pdb}";
    var destination = config.sitecoreLibraries;
	gulp.src(roots, { base: root }).pipe(
      foreach(function (stream, rootFolder) {
      	console.log("watch: " + rootFolder.path + files);
          gulp.watch(rootFolder.path + files, function (event) {
